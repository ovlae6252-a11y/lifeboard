...
- **현재 버전**: v1.1d 완료 (2026-02-18), v1.2 계획 중
...
- **Playwright** - E2E 테스트. `tests/auth.setup.ts`가 `playwright/.auth/user.json`에 인증 상태를 저장한 뒤 chromium 프로젝트가 이를 `storageState`로 사용. 테스트 실행 시 `webServer`가 자동으로 `npm run dev`를 실행함.
...
인증 상태 확인 시 `supabase.auth.getClaims()` 사용 (`getUser()` 대비 빠름, 로컬 JWT 파싱).
...
- `/auth/login` - 소셜 로그인 페이지 (Google, Kakao)
- `/auth/callback` - OAuth 콜백 Route Handler (GET, code 교환 → 세션 생성)
- `/protected/*` - 인증 필요 페이지 (Header + Footer 공통 레이아웃, `max-w-6xl`)
- `/protected` - 대시보드 (최신 뉴스 6개 프리뷰)
- `/protected/news` - 뉴스 목록 페이지 (카테고리 탭 + 검색바 + 페이지네이션)
- `/protected/news/[groupId]` - 뉴스 상세 페이지 (팩트 요약 + 관련 기사 + 북마크/공유 버튼)
- `/protected/settings` - 사용자 설정 페이지 (프로필, 선호 카테고리, 대시보드 위젯 설정)
...
- `/admin/moderation` - 콘텐츠 모더레이션 (필터 관리/품질 검토 탭)
- `/admin/users` - 사용자 관리 (역할 변경, 계정 정지)
- `/admin/monitoring` - 시스템 모니터링 (수집 로그/요약 작업/시스템 상태 탭)
- `/api/admin/moderation/filters` - 콘텐츠 필터 CRUD API
- `/api/admin/moderation/quality` - 품질 검토 API (GET/PUT approve, POST rerun)
- `/api/admin/users` - 사용자 목록 조회 API (GET, 페이지네이션 + 이메일 검색)
- `/api/admin/users/[userId]` - 사용자 수정 API (PUT, 역할 변경/계정 정지)
- `/api/admin/monitoring/logs` - 수집 로그 API (GET, 소스/상태/날짜 필터)
- `/api/admin/monitoring/jobs` - 요약 작업 API (GET/PUT retry/reset)
- `/api/admin/monitoring/status` - 시스템 상태 API (GET, lastCronRun/워커활동/DB통계)

### 컴포넌트 패턴

- `components/ui/` - shadcn/ui 기본 컴포넌트 (`npx shadcn@latest add <name>`으로 추가)
  - `sonner.tsx` - Toast 알림 (sonner 라이브러리, 테마 통합)
- `components/layout/` - 공통 레이아웃 (header, mobile-nav, footer, admin-link, nav-links 상수)
  - `header.tsx`는 Server Component. AuthButton(서버) + ThemeSwitcher/MobileNav(클라이언트) 조합
  - `footer.tsx`는 Client Component (`new Date()` 사용)
  - `nav-links.ts` - Navigation 링크 상수 (대시보드, 뉴스, 설정)
...
- `components/weather/` - 날씨 UI 컴포넌트. `weather-widget.tsx`(Server Component, 대시보드용), `hourly-forecast.tsx`, `weekly-forecast.tsx`
- `components/admin/` - 관리자 UI 컴포넌트 (모두 Client Component). `admin-sidebar.tsx`(현재 경로 활성, 5개 섹션), `stats-cards.tsx`, `pipeline-status.tsx`, `quality-metrics.tsx`, `collection-chart.tsx`/`category-chart.tsx`(Recharts), `recent-activity.tsx`, `news-source-manager.tsx`, `news-group-manager.tsx`, `news-article-manager.tsx`, `filter-manager.tsx`, `quality-review-queue.tsx`, `moderation-tabs.tsx`, `user-list.tsx`, `fetch-log-viewer.tsx`, `job-manager.tsx`, `system-status.tsx`, `monitoring-tabs.tsx`
- `auth-button.tsx` - Server Component, 반드시 `<Suspense>` 안에서 사용

### 관리자 시스템

`lib/auth/admin.ts` - 관리자 인증 유틸리티:
- `isAdmin()` - `getClaims()`의 `app_metadata.role === 'admin'` 여부 반환. 레이아웃에서 사용.
- `requireAdmin()` - 실패 시 `"관리자 권한이 필요합니다"` 에러 throw. 모든 `/api/admin/*` Route 최상단에서 호출 필수.
- `logAdminAction({ adminId, action, targetType, targetId, details })` - `admin_audit_logs` 테이블에 행위 기록. **에러 발생 시 `console.error`만 출력하고 절대 throw 하지 않음** (주 작업을 중단시키면 안 됨). 민감한 작업(숨김, 삭제, 요약 재실행)마다 호출.
...
- `getFetchLogs({ sourceId?, dateFrom?, dateTo?, status?, page, limit })` - 수집 로그 페이지네이션 조회
- `getSummarizeJobs({ status?, page, limit })` - 요약 작업 페이지네이션 조회, 그룹 제목 포함
- `getSystemStatus()` - 마지막 Cron 실행/워커 활동/테이블 레코드 수

**관리자 레이아웃 패턴** (`app/admin/layout.tsx`): `isAdmin()` 호출은 비캐시 데이터(`getClaims()`)에 접근하므로, `AdminAuthGate` async 서버 컴포넌트를 `<Suspense fallback={null}>`으로 감싸야 함. 이것이 `cacheComponents: true` 환경에서 필요한 패턴.
...
- **`"use cache"` 함수 내부 지시문 패턴**: `lib/weather/api.ts`처럼 async 함수 본문 첫 줄에 `"use cache"` 문자열 + `cacheLife({ stale, revalidate, expire })`를 사용하면 해당 함수 호출 결과가 캐시됨. `cacheTag`와 병용 가능.
...
**수집 관련 모듈:**
- `types.ts` - 파이프라인 공유 타입 (`RawArticle`, `ArticleInsert`, `NewsSource`, `FetchResult`, `GroupingResult`, `CollectResponse`)
- `rss-fetcher.ts` - RSS 피드 파싱 (`fetchRssFeed`, `toArticleInserts`)
- `normalize-title.ts` - 제목 정규화 (그룹핑 정확도 향상용)
- `og-image-fetcher.ts` - OG 이미지 일괄 수집 (`fetchOgImagesBatch`)
- `content-filter.ts` - 콘텐츠 필터링 (`shouldFilterArticle()`, `content_filters` 테이블 기반)
- `grouping.ts` - 기사 그룹핑 로직
- `grouping-queue.ts` - 비동기 그룹핑 큐 (`enqueueGroupingJob`)
- `summarize-queue.ts` - 요약 작업 큐 (`enqueueSummarizeJobs`)
- `fetch-logger.ts` - 수집 로그 기록 (`logFetchResult`, `updateLastFetchedAt`)
- `cleanup.ts` - 오래된 레코드 정리 (`cleanupOldRecords`)

**프론트엔드 쿼리 모듈** (`queries.ts`):
- `getNewsGroups({ category?, limit, offset })` - 그룹 목록 + 총 개수 (대표 기사 embedded join, FK명 `!fk_representative_article` 명시 필수)
- `getLatestNewsGroups(limit)` - 대시보드용 최신 N개
- `getNewsGroupDetail(groupId)` - 상세 페이지용 (그룹 + 대표 기사 + 팩트 요약)
- `getRelatedArticles(groupId)` - 그룹 내 관련 기사 목록
- `getUserBookmarkedGroups(userId, page, pageSize)` - 북마크 탭용 그룹 목록
- `searchNewsGroups(query, category?, page, pageSize)` - pg_trgm 유사도 검색

**카테고리 상수** (`lib/news/categories.ts`): `NEWS_CATEGORIES` 배열에 `bookmarks` 값이 포함되어 있으나 이는 **UI 전용 탭**이며 DB에는 존재하지 않음.

**수집 흐름**: Vercel Cron (하루 2회, KST 8시/20시 = UTC 23시/11시) → `/api/news/collect` → RSS 파싱 → 콘텐츠 필터링 → 중복 필터링 → DB INSERT → 그룹핑 (유사도 0.5, 72시간) → 요약 큐
...
- `news_article_groups`: `WHERE is_valid = true` partial index로 품질 검증된 그룹만 조회 최적화
...
- `docs/ROADMAP.md` - 개발 로드맵 (v1.0~v1.1d 완료, v1.2 계획)
- `docs/PRD.md` - 제품 요구사항 문서 (v3.0, 2026-02-18, v1.2 기능 명세 포함)
- `docs/ISSUE.md` - 알려진 이슈 및 버그 추적
- `docs/complete/` - 완료 버전 아카이브

...
| -------- | -------------------- | ------------------------------------------------------------------------------------------------------- | -------- | ---------------------- |
| **F100** | 소셜 로그인          | Google, Kakao, Apple 계정으로 로그인. 기존 이메일/비밀번호 인증 제거. Naver는 v1.2로 이관 (OIDC 미지원) | Must     | 로그인 페이지          |
...
### F100: 소셜 로그인 (Google, Kakao, Apple)
...
**해결**: Google, Kakao, Apple 3개 소셜 로그인 프로바이더를 지원하고, 기존 이메일/비밀번호 인증 시스템을 완전히 제거한다.
...
| **Apple**  | 네이티브      | `signInWithOAuth({ provider: 'apple' })`  | Apple Developer      |
...
| **변경 UI**     | 소셜 로그인 버튼 3개 표시 (Google, Kakao, Apple). 이메일/비밀번호 폼 완전 제거      |
| **레이아웃**    | 중앙 정렬 카드 내 Lifeboard 로고 + "소셜 계정으로 로그인" 안내 + 버튼 3개 세로 배치 |
...
| Apple      | Service ID, Key File Content | Apple Developer에서 발급      |
...
| Apple      | O        | O (최초만) | X             | 이름은 첫 로그인 시에만 전달 |
...
비로그인:
├── 홈 (/) - "시작하기" → /auth/login
└── 로그인 (/auth/login) - 소셜 로그인 버튼 (Google, Kakao, Apple)
...
| **소셜 로그인**         | F100 | Must     | 높음     | 없음             | Google/Kakao/Apple 3개. Naver는 v1.2로 이관 |
